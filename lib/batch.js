...
module.exports.config = function (settings) {
    if (settings.proxy && settings.proxy.enabled) {
        const validator = Joi.object({
                enabled: Joi.boolean(),
                url: Joi.string().uri({ scheme: [ 'http', 'https' ], allowRelative: false})
                        .required(),
                agents: Joi.object({
                    http: Joi.object().unknown().default({}),
                    https: Joi.object().unknown().default({}),
                    httpsAllowUnauthorized: Joi.object().unknown().default(false),
                }).default({})
            });
        const result = validator.validate(settings.proxy);

        const { url, agents } = result.value;
        internals.proxy = Wreck.defaults({
            baseUrl: url,
            agents: {
                https: new Https.Agent(agents.https),
                http: new Http.Agent(agents.http),
                httpsAllowUnauthorized: new Https.Agent(agents.httpsAllowUnauthorized)
            }
        });
    }
    return {
...
internals.parseResult = function (result){
    ...
    if (result instanceof Buffer){
        try {
            ...
            return internals.parseResult(string);
        } catch (e) {
            return result;
        }
    }
...
internals.dispatch = function (batchRequest, request, callback) {
    ...
    if (batchRequest.server.connections.length === 1) {
        ...
        const options = { payload: body, rejectUnauthorized: false, json: false };
        internals.proxy[request.method](path, options, (err, r, payload) => {
            let result = payload;
            if (err) {
                if (err.data) {
                    result = err.data.payload;
                } else if (err.output) {
                    result = err.output.payload;
                } else {
                    result = err;
                }
            } 
