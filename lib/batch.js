...
module.exports.config = function (settings) {
    if (settings.proxy) {
        const validator = Joi.object({
                host: Joi.string().uri({ scheme: [ 'http', 'https' ], allowRelative: false})
                        .required(),
                agents: Joi.object({
                    http: Joi.object().unknown().default({}),
                    https: Joi.object().unknown().default({}),
                    httpsAllowUnauthorized: Joi.object().unknown().default({}),
                }).optional().default({})
            });
        validator.validate(settings.proxy);

        const { host, agents } = settings.proxy;
        internals.proxy = Wreck.defaults({
            baseUrl: settings.proxy.host,
            agents: {
...
internals.dispatch = function (batchRequest, request, callback) {
    ...
    if (batchRequest.server.connections.length === 1) {
        ...
        const r = internals.proxy[request.method](path, options, (err, r, payload) => {
            const result = err ? err.data.payload : payload;
            console.log(r);
            callback({ result })
        });
    }
